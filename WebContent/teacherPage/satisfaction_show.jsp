...
<%
	...
	PreparedStatement p1 = conn.prepareStatement("select * from outschool where satisfaction='A'");
	ResultSet rs1 = p1.executeQuery();
	while(rs1.next()){
		num_A = rs1.getRow();
		chart.setValue("满意",num_A);
	}
	rs1.close();
	p1.close();
	PreparedStatement p2 = conn.prepareStatement("select * from outschool where satisfaction='B'");
	ResultSet rs2 = p2.executeQuery();
	while(rs2.next()){
		num_B = rs2.getRow();
		chart.setValue("基本满意",num_B);
	}
	rs2.close();
	p2.close();
	PreparedStatement p3 = conn.prepareStatement("select * from outschool where satisfaction='C'");
	ResultSet rs3 = p3.executeQuery();
	while(rs3.next()){
		num_C = rs3.getRow();
		chart.setValue("不满意",num_C);
	}
	rs3.close();
	p3.close();
	String filename = chart.generatePieChart(title, session, new PrintWriter(out));
